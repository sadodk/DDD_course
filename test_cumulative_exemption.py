...
def test_cumulative_exemption():
    ...
    with patch.object(
        app.visitor_service, "get_visitor_by_id", return_value=beaver_bertha
    ):
        ...
        print(
            f"      - Total: {403 * 0.21} + {1400 * 0.29} = {(403 * 0.21) + (1400 * 0.29)}"
        )
        print()
        ...
        if result2.price_amount != 490.63:
            ...
            if result2.price_amount == 442.87:
                print(
                    "   The system calculated as if this was the first visit (full exemption)"
                )
                print(
                    "   This suggests exemption state is not being maintained between visits"
                )
            else:
        ...
        with patch.object(app.visitor_service, "get_visitor_by_id", return_value=peppa):
            visit3_data = {
                "date": "2023-07-23",
                "dropped_fractions": [
                    {"amount_dropped": 1228, "fraction_type": "Construction waste"}
                ],
            ...
            print(
                f"      - Total: {1000 * 0.21} + {228 * 0.29} = {(1000 * 0.21) + (228 * 0.29)}"
            )
